...
def showResults(img, results, img_width, img_height):
    ...
    for i in range(len(results)):
        ...
        # cv2.imwrite(
        #     '/home/users/ramosrafh/git/YoloKerasFaceDetection/dataset/video/cropped/'+img_name, cropped)
...
data_path = os.path.join(frames, '*jpg')
...
for image in files:
    ...
    cv2.imwrite(
        '/home/users/ramosrafh/git/YoloKerasFaceDetection/dataset/video/cropped/'+image_split, img)
    ...
    # MALE
    if cls == 1 and not os.path.exists(male_folder+image_split) and not os.path.exists(found_folder+image_split):
        try:
            os.symlink(image, male_folder+image_split)
            os.symlink(image, found_folder+image_split)
        except FileExistsError:
            print("Deu erro!")

    # FEMALE
    elif cls == 0 and not os.path.exists(female_folder+image_split) and not os.path.exists(found_folder+image_split):
        try:
