...
def loadData(directory, video_name):
    ...
    for image in tqdm(files, desc="Files"):
        ...
        if not face:
            ...
            #cv2.imwrite('/home/ssd/rafh/save/cropped/'+image_name, img)
...
list_classes = glob.glob('/home/users/datasets/UCF-101_opticalflow/*')
arq = open("REULTADO_SEXO.txt", 'w')
for aClass in list_classes:
	video_list = glob.glob(aClass+"/*")
	...
	for video_path in video_list:
	    ...
	    for img_name, i, p in zip(image_names, idx, prob):
	        if i == 0:
	            pred_cls.append('f')
	            result["f"] +=1
	            result["pf"] +=p
	    ...
	    arq.write(video_name+";" )
	    
	    if result['pm'] > 0:
	        print("Masc", result["pm"] / result["m"])
	        arq.write("Masc; "+ str(result["pm"] / result["m"]))
	    else:
	        arq.write("Masc; "+ str(0))
	    if result['pf'] > 0:
	        print("FEM", result["pf"] / result["f"])
	        arq.write(";Fem; "+ str(result["pf"] / result["f"]))
	    else:
	        arq.write(";Fem; "+ str(0))
	    arq.write('\n')
	    print("\n\n")
