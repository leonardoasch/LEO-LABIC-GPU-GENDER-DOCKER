...
if(os.path.exists("./dataset/adience/")):
    DATASET_ROOT_PATH = ""
else:
...
for list in range(5):
    ...
    for line in lines:
        ...
        src_img = full_path
        if age_int != -1:
            shutil.copyfile(src_img, AGE_PATH+train_or_validation +
                            "/"+get_age_path(age)+"/"+str(i)+".jpg")
        shutil.copyfile(src_img, GENDER_PATH+train_or_validation +
                        "/"+get_gender_path(gender)+"/"+str(i)+".jpg")
        if age_int != -1:
            shutil.copyfile(src_img, AGE101_PATH+train_or_validation +
                            "/"+format(age, "03d")+"/"+str(i)+".jpg")
