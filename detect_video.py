...
def loadData(directory):
    ...
    for image in tqdm(files, desc="Files"):
        ...
        if not results:
            ...
            img, crop = showResults(img, results, img.shape[1], img.shape[0])
            print(crop, len(crop))
            for i in range(len(results)):
                for j in range(len(crop)):
                    # display detected face

                    startX = crop[j][0]  # xmin
                    startY = crop[j][2]  # xmax
                    endX = crop[j][1]    # ymin
                    endY = crop[j][3]    # ymax
            ...
            cv2.imwrite(
                '/home/users/ramosrafh/git/YoloKerasFaceDetection/dataset/video/cropped/'+image_name, img_copy)
...
image_names, x_test = loadData(FRAMES)
...
for img_name, i in zip(image_names, idx):
    if i == 0 and not os.path.exists(FEMALE_FOLDER+img_name) and not os.path.exists(FOUND_FOLDER+img_name):
        pred_cls.append('f')
        try:
            ...
            print("Deu erro!")
    elif i == 1 and not os.path.exists(MALE_FOLDER+img_name) and not os.path.exists(FOUND_FOLDER+img_name):
        pred_cls.append('m')
        try:
...
filenames_to_cls = list(zip(image_names, pred_cls))
